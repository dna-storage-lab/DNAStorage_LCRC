 ...
 *
 * @brief: Read identification, bit-wise majority voting, and error correction
 *  for DNA data storage with medium-scale oligo pools
 *
 *************************************/
#include <cstdlib>
#include <cstdio>
...
typedef struct
{
    int n_component;      // Number of component sequences
    uint64_t period;      // Period of composite code
    uint64_t start_phase; // Start phase of composite code

    uint64_t *M; // Parameters of Chinese Remainder Theorem
    uint64_t *M_sup;

    unsigned *cLen; // Period of component sequences
    int *cPhase;    // Initial phase of component sequences
    int **c;        // Component sequences
...
typedef struct
{
    ...
    char *log_dir;
    char *reads_file_prefix;

    CorrelationContext *corrCtx;

    pthread_mutex_t mutex;

    Configuration *config; // Composite code config
...
typedef struct
{
    ...
    unsigned *n_corrupted_ldpc;
    unsigned *n_corrupted_rs;

    double *era_codewords;
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 8)
    {
        fprintf(stderr, "Usage: "
                        "%s [result dir] [log dir] [fastq file] [Threads]"
                        "[Correlation threshold] [Mode] [file suffix ID]\n",
                argv[0]);
        return 1;
    }
    sscanf(argv[1], "%s", result_dir);
    sscanf(argv[2], "%s", log_dir);
    sscanf(argv[3], "%s", reads_file_prefix);
    sscanf(argv[4], "%d", &n_threads);
    sscanf(argv[5], "%lf", &threshold_autoCorr);
    sscanf(argv[6], "%d", &mode);
    sscanf(argv[7], "%d", &file_suffixID);
    ...
    int n_threads_file = n_threads;
    int n_threads_bwa = ceil(n_threads / n_threads);
    ...
    double era_codewords[RS_N_SHORTEN] = {0.0};
    ...
    char **rec_codeword_errfree = (char **)malloc(RS_N_SHORTEN * sizeof(char *)); // recovered LDPC-RS post LDPC decoding
    ...
    worker_hdlr.reads_file_prefix = reads_file_prefix;
    ...
    worker_hdlr.file_suffixID = file_suffixID;
    ...
    decoder_ctx.era_codewords = era_codewords;
    ...
    unsigned tot_corrupted_ldpc_iter1 = 0;
    unsigned tot_corrupted_rs_iter1 = 0;
    ...
    cons_ctx.era_codewords = era_codewords;
    cons_ctx.process_bits = decoder_ctx.n_ldpc_per_tid * LDPC_N;
    cons_ctx.n_jobs = n_threads_on;

    cons_ctx.n_ldpc_per_tid = decoder_ctx.n_ldpc_per_tid;
    ...
    time_t pre_thread_time, post_thread_time;
    ...
    pre_thread_time = current_time_ms();
    ...
    while (num_working < n_threads_file)
    {
        ...
        sprintf(reads_file, "%s%05d.fastq", reads_file_prefix, file_suffixID + cnt_batch);
        if (isFileExist(reads_file))
        {
            thpool_add_work(thpool, pipelineWorker, (void *)&worker_hdlr);
            ++cnt_batch;
            ++num_working;
            ++num_working_chip1;
        }
        ...
        if (now_time - init_time > 1000) // 1 sec detection
        {
            init_time = current_time_ms();
            break;
        }
    ...
    if (num_working_chip1)
        printf("Loaded %d fastq files from Chip-1: file-%d to file-%d!\n",
               num_working_chip1, cnt_batch - num_working_chip1 + 1, cnt_batch);
    thpool_wait(thpool);

    post_thread_time = current_time_ms();
    thread_used_time += difftime(post_thread_time, pre_thread_time);
    ...
    printf("Reads with valid primers = %u\n", tot_reads_primer_valid);
    ...
    double blk_err_rate_ldpc = 1.0 * tot_corrupted_ldpc / num_ldpc;
    double blk_err_rate_rs = 1.0 * tot_corrupted_rs / num_rs;
    ...
    if (tot_corrupted_rs)
    {
        ...
        tot_corrupted_ldpc_iter1 = 0;
        for (int tid = 0; tid < decoder_ctx.n_jobs_ldpc; ++tid)
            tot_corrupted_ldpc_iter1 += *(decoder_ctx.n_corrupted_ldpc + tid);
        ...
        tot_corrupted_rs_iter1 = 0;
        for (int tid = 0; tid < decoder_ctx.n_jobs_rs; ++tid)
            tot_corrupted_rs_iter1 += *(decoder_ctx.n_corrupted_rs + tid);

        fprintf(stderr, "\nResidual corrupted LDPC = %u/%u, corrupted RS = %u/%u\n",
                tot_corrupted_ldpc_iter1, num_ldpc, tot_corrupted_rs_iter1, num_rs);
    }
    ...
    double blk_err_rate_ldpc_iter1 = 1.0 * tot_corrupted_ldpc_iter1 / num_ldpc;
    double blk_err_rate_rs_iter1 = 1.0 * tot_corrupted_rs_iter1 / num_rs;

    post_dec_time = current_time_ms();
    dec_used_time = post_dec_time - pre_dec_time;
...
void pipelineWorker(void *args)
{
    ...
    int chip1_cnt = worker_hdlr->cnt_batch;
    ++worker_hdlr->cnt_batch;

    pthread_mutex_unlock(&worker_hdlr->mutex);

    char reads_file[500] = {0};

    sprintf(reads_file, "%s%05d.fastq", worker_hdlr->reads_file_prefix, chip1_cnt + worker_hdlr->file_suffixID);
...
void correlation_alignment(WorkerHandler *worker_hdlr, int tid, const char *reads_file)
{
    ...
    if (worker_hdlr->mode)
    {
        ...
        if ((fp_data = fopen(ofile_data, "w+")) == NULL) // read and write
        {
            fprintf(stderr, "Could not open file %s!\n", ofile_data);
        }
    ...
    if (mode)
    {
        ...
        if (n_reads_BWA > 0)
        {
            ...
            sprintf(cmdLine, "bwa mem -v 1 -t %d %s %s -o %s > /dev/null 2>&1", worker_hdlr->n_threads_bwa, refer_file_PNseq, ofile_PNseq, filepath_SAM);

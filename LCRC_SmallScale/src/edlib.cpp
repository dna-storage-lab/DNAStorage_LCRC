...
/**
 * Defines equality relation on DNAalphabet characters.
...
/**
 * Build Peq table for given query and DNAalphabet.
...
/**
 * Takes char query and char target, recognizes DNAalphabet and transforms them into unsigned char sequences
 * where elements in sequences are not any more letters of DNAalphabet, but their index in DNAalphabet.
 ...
 * @param [in] targetLength
 * @param [out] queryTransformed  It will contain values in range [0, DNAalphabet length - 1].
 * @param [out] targetTransformed  It will contain values in range [0, DNAalphabet length - 1].
...
static string transformSequences(const char* const queryOriginal, const int queryLength,
	...
	*targetTransformed = static_cast<unsigned char *>(malloc(sizeof(unsigned char) * targetLength));
	...
	// letterIdx[c] is index of letter c in DNAalphabet.
	unsigned char letterIdx[MAX_UCHAR + 1];
	bool inAlphabet[MAX_UCHAR + 1]; // inAlphabet[c] is true if c is in DNAalphabet
